...
namespace _007
{
    public partial class Form1 : Form
    {
        ...
        private void button37_Click(object sender, EventArgs e)
        {
            ...
            string Tempdata = ""; //傳資料用
            for (int i = 0; i < 36; i++)
            {
                //btn[i].Click += new EventHandler(this.buttons_Click);//1062981

                num = rnd.Next(arr.Count()); //做變數
                btn[i].Text = arr[num]; //放圖案進入卡片
                Tempdata += arr[num]; //按照順序存卡片圖案順序

                arr.RemoveAt(num);
                btn[i].ForeColor = System.Drawing.Color.Moccasin;
            }
            data = System.Text.Encoding.Unicode.GetBytes(Tempdata); //把tempdata轉成unicode存放到data
            stream = client.GetStream(); // 取得clientstream  
            stream.Write(data, 0, data.Length); //傳送
            ...
            while (true)
            {
                ...
                if (check == 1)
                {
                    ...
                    MessageBox.Show("第二位玩家已加入");
                    break;
                }
                else
                    MessageBox.Show("第二位玩家尚未加入");
            }
            Client = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
            button37.Enabled = false;
            IPEnd = new IPEndPoint(IPAddress.Parse("127.0.0.3"), 7778);
            Client.Bind(IPEnd);
            IP = (EndPoint)IPEnd;


            remoteIP = new IPEndPoint(IPAddress.Parse("127.0.0.4"), 7779); //可自行定義廣播區域跟Port
            ...
            if (e.ProgressPercentage == (int)E.recived)
            {
                ...
                foreach (string data in _Ls)
                {
                    ...
                    if (location == -1)
                    {
                        ...
                        templocat = location;
                        btn[location].ForeColor = Color.Red;
